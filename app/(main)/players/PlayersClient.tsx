...
const PlayersClient = () => {
  ...
  const { data: favorites = [], refetch: refetchFavorites } = useQuery<Player[]>({
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight pb-2 bg-linear-to-r from-blue-400 via-cyan-300 to-indigo-400 bg-clip-text text-transparent inline-block">
            Our Volleyball Players
          </h1>
          <p className="text-gray-400 mt-2 text-sm sm:text-base max-w-xl mx-auto">
            Discover the talented athletes of Aro Ekdin Volleyball Team. Check squad positions, stats and player profiles.
          </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 justify-items-center">
          {isLoading || isError
            ? [...Array(8)].map((_, i) => <PlayerCard key={i} player={null} />)
